...
export default async function Git() {
    ...
    const contributionCalendar = viewer.contributionsCollection.contributionCalendar;
    ...
    return (
        <div>
            <Suspense fallback={<Loading />}>
                <div className="bg-gray-800 text-white w-full py-6 rounded-lg text-xl mb-4">
                    <ul className="list-none space-y-2 flex items-center gap-4">
                        <div className="mx-5">
                            ...
                            />
                        </div><h1 className="text-xl font-bold text-blue-400">Repositories for {viewer.login}</h1>
                ...
                <Repositories repositories={repositories} contributionWeeks={contributionWeeks} />
            </Suspense>
        </div>
    );
}   