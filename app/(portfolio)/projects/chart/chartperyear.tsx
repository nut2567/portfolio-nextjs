...
const PopulationGrowthGraph = () => {
    ...
    useEffect(() => {
        const fetchContinents = async () => {
            try {
                ...
                //     : []
                // };
                continentData["Asia"] = [...continentData["Asia"], "East Timor", "Macao"]
        ...
        const sortedCountries = Object.entries(groupedData)
            ...
            .filter(row => {
                ...
                return !Object.keys(continentCountries).some(key => {
                    if (strikedKeys.includes(key)) { // เช็คว่า key ถูกตีตราหรือไม่

                        // ถ้าคีย์ถูกตีตรา ให้ตรวจสอบชื่อประเทศ
                        return continentCountries[key]?.includes(row) || continentCountries[key].includes(row);
                    }
    ...
    const handleClick = (key: string) => {
        setStrikedKeys((prevStrikedKeys) =>
        ...
        <div>
            ...
            <div className="flex gap-5 mr-6 items-center justify-center">
                ...
                {Object.keys(continentCountries).map((key, index) => (
                    <div
                        key={index}
                        className={`flex items-center cursor-pointer p-4 inline-block ${strikedKeys.includes(key) ? 'line-through' : ''
                            }`}
